...
# + Pump.fun live feed (subscribeNewToken) con tabella linkata e alert opzionali
# + Meme Score, Watchlist, Export CSV, Pair Age, Grafici
...
from pump_feed import PumpFeed  # <--- nuovo
...
SEARCH_QUERIES = [
    ...
    "chain:solana usdc","chain:solana usdt","chain:solana sol",
    "chain:solana bonk","chain:solana wif","chain:solana pepe",
]
...
with st.sidebar:
    ...
    st.divider()
    st.subheader("Pump.fun (live)")
    enable_pump = st.toggle("Attiva feed Pump.fun (nuove coin)", value=True)
    pump_rows_to_show = st.number_input("Quante righe mostrare", min_value=10, max_value=200, value=40, step=10)
    pump_alert_new = st.toggle("Alert Telegram su nuove coin Pump.fun", value=False)
    pump_api_key = st.text_input("PumpPortal API key (solo per streams a pagamento, opz.)", value=os.getenv("PUMP_API_KEY",""))
...
def avg(lst):
    xs = [x for x in lst if x is not None]; return (sum(xs)/len(xs)) if xs else None
def fmt_int(n): return f"{int(round(n)):,}".replace(",", ".") if n is not None else "N/D"
def hours_since_ms(ms):
    if not ms: return None
    try: return max(0.0, (time.time() - int(ms)/1000.0) / 3600.0)
    except Exception: return None
def ms_to_dt(ms):
    if not ms: return ""
    try: return datetime.datetime.utcfromtimestamp(int(ms)/1000).strftime("%Y-%m-%d %H:%M")
    except Exception: return str(ms)
def fmt_age(hours):
    if hours is None: return ""
    if hours < 1: return f"{int(round(hours*60))}m"
    if hours < 48: h = int(hours); m = int(round((hours - h) * 60)); return f"{h}h {m}m"
    d = int(hours // 24); h = int(hours % 24); return f"{d}d {h}h"
def norm_list(s):
    out = []; 
    for part in (s or "").replace(" ", "").split(","):
        if part: out.append(part.upper() if len(part) <= 8 else part)
    return out
...
def safe_sort(df, col, ascending=False):
    return df.sort_values(by=[col], ascending=ascending) if (df is not None and not df.empty and col in df.columns) else df
...
def score_age(hours): return 0.5 if hours is None else max(0.0, min(1.0, 1.0 - (hours / 72.0)))
...
def compute_meme_score_row(r, weights, sweet_min, sweet_max):
    ...
    f = (weights[0]*score_symbol(base) + weights[1]*score_age(ageh) + weights[2]*s_sigmoid(tx1)
         + weights[3]*score_liq(liq, sweet_min, sweet_max) + weights[4]*score_dex(dex))
    return round(100.0 * f / max(1e-6, sum(weights)))
...
if disable_all_filters:
    provider.set_filters(only_raydium=False, min_liq=0, exclude_quotes=set())
else:
    provider.set_filters(only_raydium=only_raydium, min_liq=min_liq, exclude_quotes=set(exclude_quotes))

# Pump feed init/stop
if enable_pump:
    if "pump_feed" not in st.session_state:
        st.session_state["pump_feed"] = PumpFeed(api_key=pump_api_key, max_items=300)
        st.session_state["pump_feed"].start()
else:
    if "pump_feed" in st.session_state:
        st.session_state["pump_feed"].stop(); del st.session_state["pump_feed"]
...
if watchlist_only and not df_view.empty:
    df_view = df_view[df_view.apply(is_watch_hit_row, axis=1)].reset_index(drop=True)
...
# ---------------- Nuove coin – Birdeye + Fallback provider ----------------
...
if vol24_avg is not None and vol24_avg > 0:
    score = "ON FIRE" if vol24_avg > 1_000_000 else ("MEDIO" if vol24_avg > 200_000 else "FIACCO")
...
if not df_pairs.empty:
    ...
    if not show_pair_age and "Pair Age" in display_cols: display_cols.remove("Pair Age")
    st.dataframe(
    ...
    csv_bytes = df_pairs[[c for c in df_pairs.columns if c not in ["_pairAddress"]]].to_csv(index=False).encode("utf-8")
...
# ---------------- Pump.fun: sezione live ----------------
st.markdown("### New on Pump.fun (live)")

def _fmt_timeago(ts_ms):
    try:
        sec = max(0, time.time() - (int(ts_ms)/1000.0))
...
feed_status = "disabilitato"
rows = []
if enable_pump and "pump_feed" in st.session_state:
    pf: PumpFeed = st.session_state["pump_feed"]
    rows = pf.snapshot(limit=pump_rows_to_show)
    feed_status = "connesso ✅" if pf.connected else "in reconnect…"
else:
    feed_status = "disabilitato"
...
if rows:
    df_pump = pd.DataFrame([{
        ...
        "Symbol": r["symbol"],
        "Mint": r["mint"],
        "When": _fmt_timeago(r["timestamp"]),
        "Pump.fun": r["pumpUrl"],
        "DexScreener": r["dexsUrl"],
        "_mint": r["mint"],
    } for r in rows])
    st.dataframe(
        df_pump.drop(columns=["_mint"]),
        use_container_width=True,
        column_config={
            "Pump.fun": st.column_config.LinkColumn("Pump.fun"),
            "DexScreener": st.column_config.LinkColumn("DexScreener"),
        }
    )
else:
    st.info("Ancora nessun evento da Pump.fun (o feed appena avviato).")

st.caption(f"Feed Pump.fun: {feed_status} • Fonte WS ufficiale (subscribeNewToken)")

# ---------------- Alert Telegram (pairs & nuovi token) ----------------
...
def should_alert(row):
    ...
    if alert_meme_min > 0: cond = cond and (row["Meme Score"] >= alert_meme_min)
    return cond
...
if enable_alerts and TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID and not df_pairs.empty:
    ...
    for r in alerts_to_send:
        text = (f"🔥 *Meme Radar Trigger*\n"
                f"Pair: {r['Pair']} ({r['DEX']})\n"
                f"Meme Score: {r['Meme Score']}\n"
                f"Txns 1h: {r['Txns 1h']} • Liq: ${r['Liquidity (USD)']:,}\n"
                f"Vol24h: ${r['Volume 24h (USD)']:,}\n"
                f"{r['Link']}")
        if send_telegram(TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, text):
            st.session_state["sent_alerts"].add(str(r.get("_pairAddress","")))

# Alert sui NUOVI token Pump.fun (facoltativo)
if pump_alert_new and enable_pump and TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID and "pump_feed" in st.session_state:
    pf: PumpFeed = st.session_state["pump_feed"]
    # invia solo i primissimi 1-3 della tabella, per evitare spam
    for r in rows[:3]:
        key = f"pump:{r['mint']}"
        if key in st.session_state["sent_alerts"]: continue
        short = f"{r['name']} ({r['symbol']})" if r['symbol'] else r['name']
        text = f"🆕 Pump.fun: {short}\n{r['pumpUrl']}\nDexScreener: {r['dexsUrl']}"
        if send_telegram(TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, text):
...
with d2: st.text(f"Righe provider (post-filtri provider): {len(df_provider)}")
with d3: st.text(f"Watchlist attiva: {bool(watchlist)} • Righe dopo watchlist: {post_count}")
with d4:
    pf_stat = "on" if (enable_pump and 'pump_feed' in st.session_state and st.session_state['pump_feed'].connected) else "off"
    st.text(f"Pump.fun feed: {pf_stat} • Source: {'Birdeye' if bird_tokens else 'Dex fallback'}")
